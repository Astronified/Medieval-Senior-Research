...
def validate_coords(input_field, original_info, text_entity, confirm_btn, map_entity2):
    ...
    show_population_ui(final_coords, map_entity2)
...
def show_population_ui(final_coords, map_entity2):
    """
    ...
    for s_idx in range(3):
        ...
        lbl = Text(
            parent=camera.ui,
            text=SETTLEMENT_NAMES[s_idx],
            x=cx, y=0.37, scale=1.4,
    ...
    note = Text(
        parent=camera.ui,
        text='Regional multipliers: S1=1.3  S2=0.9  S3=1.5',
    ...
    confirm = MenuButton(
        ...
        on_click=Func(lambda: confirm_population(
            pop_inputs, job_inputs, final_coords, map_entity2, confirm
        ))
    )
    _pop_ui_entities.append(confirm)


def confirm_population(pop_inputs, job_inputs, final_coords, map_entity2, confirm_btn):
    confirm_btn.disable()

    JOBS     = ['Farmer', 'Soldier', 'Clergy', 'Child']
    REGIONAL = [1.3, 0.9, 1.5]
    LAND     = 1500
    ...
    for s_idx in range(3):
        ...
        settlement_configs.append({
            ...
            'land'                : LAND,
            'population'          : pop,
            'regional_multiplier' : REGIONAL[s_idx],
