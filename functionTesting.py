...
def function_testing(birthChance, deathChance, harvest_std, crowding_factor):
    class Settlement:
        ...
        def passOneSeason(self):
            ...
            raw_multiplier = np.random.normal(1.8, harvest_std)
                ...
                if not dying:
                    if person.age > 50 * 12:
                        if random.random() < (person.age - 50 * 12) / (20 * 12) * 0.1:
                            dying = True
                    death_rate = deathChance if not isinstance(person, Child) else 0.006
    ...
    # print("fail rate: " + str(failed/100))
    fail_rate1 = (failed/100)
    denom = (100 - failed) if (100 - failed) > 0 else 1
    avgChangeJax = (averageEndStart/denom)
    return {
        "fail_rate": fail_rate1,
        "avg_change": avgChangeJax
    }
